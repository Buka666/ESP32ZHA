...
- Для запуска привязки (network steering) добавлена кнопка на `GPIO9` (кнопка `BOOT` на большинстве ESP32-C6 SuperMini).
- Кнопка активна по уровню `LOW` и опрашивается с антидребезгом.
- При нажатии запускается Zigbee commissioning, после чего устройство можно добавить в ZHA.
